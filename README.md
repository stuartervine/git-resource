...
* `retrieve_tags`: *Optional.* Defaults to `false`. If specified as `true` the 
  resource will retrieve all tags on clone / fetch. If `false` git will
  clone using the flag `--single-branch`. 
